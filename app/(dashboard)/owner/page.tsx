...
function expiryBadge(expiry: string) {
  ...
  const days = Math.floor((new Date(expiry).getTime() - Date.now()) / 86400000);
  if (s === "expired")
    return { cls: "bg-danger-bg text-danger-text", dot: "#991B1B", label: `${Math.abs(days)}d overdue` };
  if (s === "soon")
    return { cls: "bg-warning-bg text-warning-text", dot: "#92400E", label: `${days}d left` };
...
export default async function OwnerDashboard() {
  ...
  let pickUpName = "";
  if (pickUp?.submitted_by) {
    const { data: pu } = await service.from("profiles").select("full_name").eq("id", pickUp.submitted_by).maybeSingle();
    pickUpName = (pu as { full_name?: string } | null)?.full_name ?? "a teammate";
  }
    ...
    <PageShell role="owner" title="Operations Dashboard" subtitle="Manage your team, clients, and compliance" userName={profile?.full_name}>
      <div className="space-y-4 max-w-6xl">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <Link href="/owner/forms" className="bg-paper border border-line2 rounded-xl p-5 block hover:border-gold transition">
          ...
          <Link href="/owner/licenses" className="bg-paper border border-line2 rounded-xl p-5 block hover:border-gold transition">
            ...
            <p className={`text-[12px] mt-1 ${(licenseAttention ?? 0) > 0 ? "text-warning-text" : "text-ink3"}`}>
              {licenseAttention ?? 0} need attention
            </p>
          </Link>
          <Link href="/owner/documents" className="bg-paper border border-line2 rounded-xl p-5 block hover:border-gold transition">
        ...
        {pickUp && (
          <div className="bg-navy rounded-xl p-5 flex items-center justify-between gap-4">
            <div className="min-w-0">
              ...
              <p className="text-white text-[13px] font-medium mt-1 capitalize">{pickUp.form_type.replace(/_/g, " ")}</p>
              <p className="text-white/50 text-[12px] mt-0.5">Started by {pickUpName} · {relativeTime(pickUp.created_at)}</p>
            </div>
            <Link href="/owner/forms" className="text-gold border border-gold rounded-lg px-4 py-2 text-[13px] font-medium flex-shrink-0">
              Open →
            </Link>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div key={log.id} className="flex items-center gap-3 py-2">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-[12px] text-muted truncate">
                          {actorMap.get(log.actor_id ?? "") ?? "System"} · {relativeTime(log.created_at)}
                        </p>
                      </div>
          ...
          <div className="bg-paper border border-line2 rounded-xl p-5">
            <div className="flex items-center justify-between mb-2">
              ...
              <Link href="/owner/licenses" className="text-[12px] text-gold">See all →</Link>
            </div>
            {licenses.length === 0 ? (
              ...
              {careRows.map((c) => {
                ...
                return (
                  <div key={c.clientId} className="flex items-center gap-3 py-2">
                    <span
                      style={{ width: 7, height: 7, borderRadius: "50%", backgroundColor: isStale ? "#991B1B" : "#065F46" }}
                    ...
                    <span className="text-[12px] text-muted w-24 text-right">
                      {c.status === "no_plan" ? "Never" : `${c.daysSincePlan}d ago`}
                    </span>
                    {isStale ? (
                      <Link
                        href={`/owner/forms?open=client_care_plan&prefill_name=${encodeURIComponent(c.clientName)}`}
                        className="text-gold text-[12px] w-24 text-right"
                      >
                        Send form →
                      </Link>
                    ) : (
